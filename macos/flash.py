#!/usr/bin/env python3
"""
LAFVIN ESP32-S3 AI Chatbot Flash Tool (Linux/Ubuntu Compatible)
...
def find_port():
    """Auto-detect the ESP32 serial port on Linux and macOS."""
    # Added Linux paths (/dev/ttyUSB* and /dev/ttyACM*) alongside macOS paths
    candidates = (
    ...
    print(f"Detected ports: {candidates}")
...
def main():
    ...
    if not ports:
        ...
        # Updated error message to display generic tty devices
        print("   Available ports: " + str(glob.glob("/dev/tty*")))
        sys.exit(1)
